// src/pages/Documents/Books/Book.jsx
...
import { getAllBooks, getBookCopies } from "../../../services/bookService";
import ButtonLoader from "../../../components/Loading/ButtonLoader";
import BookDetailPanel from "./BookDetailPanel";

const ITEMS_PER_PAGE = 5; // mặc định 5 bản ghi / trang
...
// Map trạng thái bản sao → màu Chip
const statusColor = (s) => {
...
export default function Book() {
    ...
    // Drawer state (panel chi tiết)
    const [detailOpen, setDetailOpen] = useState(false);
    const [selectedId, setSelectedId] = useState(null);
    const [selectedBook, setSelectedBook] = useState(null);

    // Expand state cho từng sách
    const [expanded, setExpanded] = useState({}); // { [documentId]: true/false }
    const [copiesMap, setCopiesMap] = useState({}); // { [documentId]: { loading, error, data } }

    const fetchAll = async () => {
        try {
            setLoading(true);
            setError("");
            const list = await getAllBooks(); // api.js tự gắn token
            setBooks(list);
            setCurrentPage(1);
        } catch (e) {
    ...
    const openDetail = (book) => {
        ...
        setSelectedBook(book); // có sẵn dữ liệu cho panel hiển thị tức thì
        setDetailOpen(true);
    };

    // Toggle expand 1 hàng; fetch copies lần đầu
    const toggleExpand = async (book) => {
        const id = book.documentId;
        setExpanded((prev) => ({ ...prev, [id]: !prev[id] }));

        // nếu chưa có dữ liệu copies → gọi API
        if (!copiesMap[id]) {
...
/** --------------------- Hàng + Collapse ---------------------- */
function FragmentRow({ book, isOpen, onToggle, onOpenDetail, copiesState }) {
    const id = book.documentId;
        ...
        <>
            ...
            <TableRow hover>
                ...
                <TableCell sx={{ maxWidth: 520 }}>
                    <Typography fontWeight={600} noWrap title={book.title}>
                        {book.title}
                    </Typography>
                </TableCell>
                                            ...
                                            copiesState.data.copies.map((c) => {
                                                ...
                                                return (
                                                    <TableRow key={c.documentCopyId} hover>
                                                        ...
                                                        <TableCell>
                                                            <Typography fontFamily="monospace">{c.barCode}</Typography>
                                                        </TableCell>
                                                        <TableCell>
                                                            <Chip size="small" color={statusColor(c.status)} label={c.status} />
                                                        </TableCell>
                                                        ...
                                                        <TableCell align="center" width={64}>
                                                            <Tooltip title="Tùy chọn">
                                                                <IconButton
                                                                    size="small"
                                                                    onClick={() => console.log("More actions for copy:", c.documentCopyId)}
                                                                >
