// src/pages/Documents/Magazines/EditMagazineDialog.jsx
...
import { getMagazineById, updateMagazine } from "../../../services/magazineService";
...
export default function EditMagazineDialog({ open, id, initialItem, onClose, onUpdated }) {
  const { enqueueSnackbar } = useSnackbar();

  // dữ liệu gốc để hiển thị preview
  ...
  // form sửa – chỉ set những field thay đổi
  ...
  const setMD = (k, v) =>
    setForm((p) => ({ ...p, magazineData: { ...(p.magazineData || {}), [k]: v } }));
  ...
  const rmAuthor = (i) => {
    setAuthorsUI((p) => {
      const next = p.filter((_, idx) => idx !== i).map((a, idx) => ({ ...a, ord: idx + 1 }));
      return next;
    });
    setAuthorsTouched(true);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!initialItem) {
          const full = await getMagazineById(id);
          setBase(full);
          ...
          setBase(initialItem);
          const au = (initialItem.authors || []).slice().sort((x, y) => (x.ord ?? 0) - (y.ord ?? 0))
            .map(a => ({ fullName: a.fullName || "", role: a.role || "main", ord: a.ord ?? 1 }));
          setAuthorsUI(au.length ? au : [{ fullName: "", role: "main", ord: 1 }]);
        ...
        setError(e.message || "Không tải được dữ liệu tạp chí.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    try {
      ...
      if (!payload.ebookFile && typeof payload.ebookViewUrl === "string" && payload.ebookViewUrl.trim()) {
        ...
        if (!(u.endsWith(".pdf") || u.endsWith(".epub"))) {
          throw new Error("URL tài liệu phải là PDF hoặc EPUB (.pdf/.epub).");
        }
      }
      if (payload.ebookFile) {
        ...
        const okExt = SUPPORTED_EBOOK_EXT.some((ext) => name.endsWith(ext));
        if (!(okMime || okExt)) {
          throw new Error("Tệp chỉ hỗ trợ PDF hoặc EPUB.");
        }
      }

      const res = await updateMagazine(id, payload);
      if (!res?.ok) throw new Error(res?.message || "Cập nhật thất bại.");

      enqueueSnackbar("Cập nhật tạp chí thành công!", { variant: "success" });
      onUpdated?.(res.data);
      onClose?.();
    } catch (e) {
                        ...
                        <Stack spacing={2}>
                          ...
                          <Grid container spacing={2}>
                            <Grid item xs={12} sm={4}>
                              <TextField
                                label="ISSN"
                                value={form.magazineData?.issn ?? base.magazineData?.issn ?? ""}
                                onChange={(e) => setMD("issn", e.target.value)}
                            ...
                            <Grid item xs={12} sm={4}>
                              <TextField
                                label="Số phát hành"
                                value={form.magazineData?.issueNumber ?? base.magazineData?.issueNumber ?? ""}
                                onChange={(e) => setMD("issueNumber", e.target.value)}
                                fullWidth
                              />
                            </Grid>
                            <Grid item xs={12} sm={4}>
                              <TextField
                                ...
                                type="date"
                                value={form.magazineData?.releaseDate ?? base.magazineData?.releaseDate ?? ""}
                                onChange={(e) => setMD("releaseDate", e.target.value)}
